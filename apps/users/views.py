...
class UserViewset(CreateModelMixin,viewsets.GenericViewSet,mixins.RetrieveModelMixin,mixins.UpdateModelMixin):
    ...
    #这里需要动态选择用哪个序列化方式
    #1、UserRegSerializer（用户注册），只返回username和mobile，会员中心页面需要显示更多字段，所以要创建一个UserDatailSerializer
    #2、问题来了，如果注册的使用userdatailserializer，又会导致验证失败，所以需要动态的使用serilaizer
